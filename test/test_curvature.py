...
def test_curvature(env_fn, trainer):
    ...
    results = trainer.calculate_eigenvalues(100, 1.0e-5)
...
if __name__ == "__main__":
    ...
    test_curvature(
        continious_env_fn,
        SB3OffPolicyTrainer(
            continious_env_fn, ExtSAC("MlpPolicy", continious_env_fn(), device="cuda")
        ),
    )
    ...
    test_curvature(
        discrete_env_fn,
        SB3OnPolicyTrainer(
            discrete_env_fn, ExtA2C("MlpPolicy", discrete_env_fn(), device="cpu")
        ),
    )
    ...
    test_curvature(
        discrete_env_fn,
        SB3OnPolicyTrainer(
            discrete_env_fn, ExtPPO("MlpPolicy", discrete_env_fn(), device="cpu")
        ),
    )
    print("testing Rainbow curvature")
    test_curvature(
        discrete_env_fn, RainbowTrainer("space_invaders", learning_starts=1000)
    )
