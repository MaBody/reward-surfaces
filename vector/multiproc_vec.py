...
def async_loop(
    vec_env_constr, pipe, shared_obs, shared_actions, shared_rews, shared_dones
):
    try:
...
class ProcConcatVec:
    def __init__(self, vec_env_constrs, observation_space, action_space, tot_num_envs):
        ...
        self.shared_obs = SharedArray(
            (num_envs,) + self.observation_space.shape,
            dtype=self.observation_space.dtype,
        )
        act_space_wrap = SpaceWrapper(self.action_space)
        self.shared_act = SharedArray(
            (num_envs,) + act_space_wrap.shape, dtype=act_space_wrap.dtype
        )
