...
def gradtensor_to_npvec(params, include_bn=True):
    ...
    return np.concatenate(
        [p.grad.data.cpu().numpy().ravel() for p in params if filter(p)]
    )
...
def npvec_to_nplist(vec, params):
    ...
    for p in params:
        numel = p.data.numel()
        rval.append(
            vec[loc : loc + numel].reshape(tuple(p.data.shape)).astype(np.float32)
        )
        loc += numel
...
def calculate_est_hesh_eigenvalues(estimator, num_samples, tol):
    ...
    assert (
        maxeig >= 0 or mineig < 0
    ), "something weird is going on but this case is handled by the loss landscapes paper, so duplicating that here"
