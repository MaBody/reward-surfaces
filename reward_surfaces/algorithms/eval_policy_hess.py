...
def gen_advantage_est_episode(rews, vals, decay, gae_lambda=1.0):
...
def gen_advantage_est(rewards, values, decay, gae_lambda=1.0):
    return [
        gen_advantage_est_episode(rew, val, decay, gae_lambda)
        for rew, val in zip(rewards, values)
    ]
...
def gather_policy_hess_data(
    ...
    while not done or (len(episode_rewards) < num_episodes and tot_steps < num_steps):
        _, original_rew, done, value, state, act, info = (
            evaluator._next_state_act()
        )  # , deterministic=True)
...
def get_used_params(evaluator, states, actions):
    ...
    grads = torch.autograd.grad(
        out, inputs=params, create_graph=True, allow_unused=True
    )
...
def compute_grad_mags(evaluator, params, all_states, all_returns, all_actions):
    ...
    for eps in range(len(all_states)):
        ...
        for idx in range(0, len(eps_act), batch_size):
            ...
            batch_states = torch.squeeze(
                torch.tensor(eps_states[idx : idx + eps_batch_size], device=device),
                dim=1,
            )
            batch_actions = torch.tensor(
                eps_act[idx : idx + eps_batch_size], device=device
            ).reshape(eps_batch_size, -1)
            batch_returns = torch.tensor(
                eps_returns[idx : idx + eps_batch_size], device=device
            ).float()
...
def compute_grad_mags_batch(
    evaluator, params, action_evaluator, num_episodes, num_steps
):
    ...
    while episode_count < num_episodes and step_count < num_steps:
        ...
        while not done:
            _, original_reward, done, value, state, act, info = (
                evaluator._next_state_act()
            )  # , deterministic=True)
        ...
        episode_returns = np.ones_like(episode_rewards) * (
            sum(episode_rewards) - baseline
        )
        episode_count += 1
        ...
        for idx in range(0, len(episode_actions), batch_size):
            ...
            batch_states = torch.tensor(
                np.array(episode_states[idx : idx + clipped_batch_size]), device=device
            )
            batch_actions = torch.tensor(
                np.array(episode_actions[idx : idx + clipped_batch_size]), device=device
            )
            batch_returns = torch.tensor(
                np.array(episode_returns[idx : idx + clipped_batch_size]), device=device
            )
...
def compute_policy_gradient(evaluator, all_states, all_returns, all_actions, device):
    ...
    params = get_used_params(
        evaluator,
        torch.squeeze(torch.tensor(all_states[0][0:2], device=device), dim=1),
        torch.tensor(all_actions[0][0:2], device=device),
    )

    grad_mag, grad_dir = compute_grad_mags(
        evaluator, params, all_states, all_returns, all_actions
    )
...
def compute_policy_gradient_batch(evaluator, action_evaluator, num_episodes, num_steps):
    ...
    # torch.squeeze is used to fix atari observation shape
    test_states, test_returns, test_actions = gather_policy_hess_data(
        evaluator, 2, num_steps, action_evaluator.gamma, "UNUSED", gae_lambda=1.0
    )

    params = get_used_params(
        action_evaluator,
        torch.squeeze(
            torch.tensor(np.array(test_states[0][0:2]), device=device), dim=1
        ),
        torch.tensor(np.array(test_actions[0][0:2]), device=action_evaluator.device),
    )

    grad_mag, grad_dir = compute_grad_mags_batch(
        evaluator, params, action_evaluator, num_episodes, num_steps
    )
...
def compute_vec_hesh_prod(
    evaluator, params, all_states, all_returns, all_actions, vec, batch_size=512
):
    device = params[0].device
    ...
    for eps in range(len(all_states)):
        ...
        for idx in range(0, len(eps_act), batch_size):
            ...
            grads = torch.autograd.grad(
                outputs=logprob, inputs=tuple(params), create_graph=True
            )

            logprob_mul_return = torch.dot(
                evaluator.eval_log_prob(batch_states, batch_actions), batch_returns
            )
            grad_mul_ret = torch.autograd.grad(
                outputs=logprob_mul_return, inputs=tuple(params), create_graph=True
            )
            assert len(vec) == len(grads)
            g_mr_dot_v = sum(
                [
                    torch.dot(g_mr.view(-1), v.view(-1))
                    for g_mr, v in zip(grad_mul_ret, vec)
                ],
                torch.zeros(1, device=device),
            )

            hesh_prods = torch.autograd.grad(
                g_mr_dot_v, inputs=params, create_graph=True
            )
...
def calculate_true_hesh_eigenvalues(
    evaluator, all_states, all_returns, all_actions, tol, device
):
    ...
    params = get_used_params(
        evaluator,
        torch.squeeze(torch.tensor(all_states[0][0:2], device=device)),
    ...
    def hess_vec_prod(vec):
        ...
        accum = compute_vec_hesh_prod(
            evaluator, params, all_states, all_returns, all_actions, vec
        )
